...
describe("camada de apresentação — cobertura fora do painel", () => {
  ...
  it("traduz o vocabulário técnico restante exposto ao usuário", () => {
    ...
    // Perfil que constava em PERFIS_DE_VINCULO e aparecia cru no seletor.
    expect(rotulo("SERVICE_ACCOUNT")).toBe("CONTA DE SERVIÇO");
    // Os estados da reconciliação NÃO entram nesta base: a tela não existe.
    expect(rotulo("EXACT_UNIQUE")).toBe("EXACT_UNIQUE");
  });
